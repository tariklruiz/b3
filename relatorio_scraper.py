...
def _grid_params_cnpj(cnpj: str, cutoff_str: str | None) -> dict:
    """
    Build query params for a per-CNPJ grid query. Minimal — only `cnpjFundo`,
    ordering, and the document-type filters. We deliberately drop `dataInicial`
    since the date filter showed no benefit and may be a scraper signature.

    Why per-CNPJ (not bulk) on KingHost's IP:
      - Smaller responses (15-25 docs per fund) vs ~5,300 bulk rows we'd
        throw most of away
      - Faster total discovery: ~60 small calls vs ~75 paginated bulk calls
      - One fund's failure doesn't poison the whole pass
      - No fund_listing matching needed — CVM filters server-side
...
def scan_grid_for_cnpj(session: requests.Session, cnpj: str, ticker: str,
    ...
    """
    Fetch the most-recent 25 relatórios for a single CNPJ. CVM filters
    server-side by cnpjFundo and orders by dataReferencia desc, so the
    first page already contains M/M-1/M-12 for monthly publishers.

    Returns raw grid rows. `since_str` is ignored (kept for API stability).
    """
    params = {
...
def process_pass(tipo_fundo: str, tipo_fundo_id: int, since_str: str) -> dict:
    """
    Run a single FII or FIAGRO pass with INTERLEAVED per-CNPJ discovery and
    download.

    For each fund in the universe:
      1. Query CVM grid filtered by that fund's CNPJ (discovery)
    ...
    Why interleave instead of discover-all-then-download-all:
      - Resilience: a network blip mid-run loses one fund's progress, not
        the entire universe's discovery work.
      - Less bot-like: a real user searches one fund, downloads its files,
        searches the next. Bulk-list-then-bulk-download is a scraper
        signature.
      - Visibility: each fund's progress logs contiguously, easier to read.

    tipo_fundo_id is kept for backward compat; CNPJ uniquely identifies the
    fund without needing tipoFundo.
    """
    ...
    if not universe:
        ...
        return {"label": tipo_fundo, "scanned": 0, "new_docs": 0,
                "downloaded": 0, "errors": 0}

    log.info(f"[{tipo_fundo}] Universe: {len(universe)} CNPJs -- per-fund discover+download")

    session = requests.Session()
    session.headers.update(BROWSER_HEADERS)

    # Warm up against the public landing page once per pass to populate
    # session cookies before any API call.
    ...
    for idx, (cnpj, info) in enumerate(universe.items(), start=1):
        ...
        # reading search results before clicking download.
        jittered_sleep()
        ...
        if not new_for_fund:
            log.info(f"[{tipo_fundo}]   nothing new for {ticker} (already in DB or 0 docs)")
            continue
        ...
        for doc in new_for_fund:
            ...
            if ok:
                downloaded += 1
                # Mark seen so retry of the same pass doesn't re-download
                seen.add(doc["doc_id"])
            if err:
                errors += 1
